...
//#region node_modules/.nitro/vite/services/ssr/assets/ai-queries-D3oXUHn6.js
...
function useSetApplicationStatus() {
	...
	return useMutation({
		mutationFn: async ({ internshipId, status }) => {
			const { data: internship } = await supabase.from("internships").select("title, posted_by").eq("id", internshipId).single();
			const { data: profile } = await supabase.from("student_profiles").select("full_name").eq("user_id", user.id).single();
...
function useUpdateApplicationByRecruiter(internshipId) {
	const qc = useQueryClient();
	return useMutation({
		mutationFn: async ({ candidateId, status, notes }) => {
			const { data: internship } = await supabase.from("internships").select("title, company_name").eq("id", internshipId).single();
			const updates = { status };
			if (notes !== void 0) updates.notes = notes;
			const { error } = await supabase.from("applications").update(updates).eq("user_id", candidateId).eq("internship_id", internshipId);
			if (error) throw error;
			let msg = `Your application for "${internship?.title}" at "${internship?.company_name}" has been updated to "${status}".`;
			if (notes) msg += ` Message from recruiter: "${notes}"`;
			await supabase.from("notifications").insert([{
				user_id: candidateId,
...
//#endregion
export { useSetFeedback as C, useToggleSave as D, useTestAttempts as E, useUpdateApplicationByRecruiter as O, useSetApplicationStatus as S, useStudentProfile as T, useRejectionAnalyses as _, useCandidates as a, useRemoveApplication as b, useFeedback as c, useInternships as d, useMarkNotificationRead as f, useParseResume as g, useNotifications as h, useAssistantSend as i, useUpdateStudentProfile as k, useGenerateTest as l, useMyInternships as m, useApplications as n, useClearNotifications as o, useMyCompany as p, useAssistantMessages as r, useDismissReminder as s, useAnalyzeRejection as t, useGradeTest as u, useReminderDismissals as v, useSetReminderPrefs as w, useSaved as x, useReminderPrefs as y };
