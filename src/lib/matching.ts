...
export function scoreInternship(
  ...
  const domainInterest =
    profile.interests.some((i) => norm(i) === norm(internship.domain)) ||
    profile.preferred_domains.some((d) => norm(d) === norm(internship.domain));
  const matchedInterests = overlap(
    [internship.domain, ...internship.skills],
    [...profile.interests, ...profile.preferred_domains],
  );

  const specialisationHit =
    !!profile.specialization &&
    (norm(internship.domain).includes(norm(profile.specialization)) ||
      norm(profile.specialization).includes(norm(internship.domain)));

  const locationMatch =
    profile.preferred_locations.length === 0 ||
    profile.preferred_locations.some(
      (l) => norm(internship.location).includes(norm(l)) || norm(l).includes(norm(internship.location)),
    );
  const workModeMatch = !profile.work_mode || profile.work_mode === internship.work_mode;
  const durationMatch =
    !profile.duration || profile.duration === "Flexible" || norm(internship.duration).includes(norm(profile.duration).split(" ")[0] ?? "");
  const stipendMatch =
    !profile.min_stipend || internship.stipend == null || internship.stipend >= profile.min_stipend;

  const behaviourBoost =
    (savedDomains.filter((d) => norm(d) === norm(internship.domain)).length > 0 ? 6 : 0) +
    (appliedDomains.filter((d) => norm(d) === norm(internship.domain)).length > 0 ? 4 : 0);
  ...
  if (matchedSkills.length)
    reasons.push(
      `You have ${matchedSkills.slice(0, 3).join(", ")}${matchedSkills.length > 3 ? " and more" : ""} from the required skills`,
    );
  ...
  const reason =
    matchedSkills.length || domainInterest
  ...
  return { internship, score, matchedSkills, matchedInterests, reason, reasons };
}
